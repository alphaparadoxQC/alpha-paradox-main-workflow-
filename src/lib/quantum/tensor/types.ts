/**
 * ============================================================
 * TENSOR NETWORK TYPES
 * ============================================================
 * Type definitions for Matrix Product State (MPS) tensor network
 * representation of quantum states.
 * ============================================================
 */

import { Complex } from '../complex';

/**
 * A tensor in the MPS representation
 * For a single qubit at position i:
 * - Shape: [left_bond, physical_dim, right_bond]
 * - physical_dim = 2 (|0⟩ and |1⟩)
 * - bond dimensions grow with entanglement
 */
export interface MPSTensor {
  data: Complex[][][]; // [left_bond][physical][right_bond]
  leftBond: number;
  rightBond: number;
}

/**
 * Matrix Product State representation
 * Represents an n-qubit state as a chain of tensors
 */
export interface MPS {
  tensors: MPSTensor[];
  qubitCount: number;
  maxBondDimension: number;
}

/**
 * SVD result for tensor decomposition
 */
export interface SVDResult {
  U: Complex[][];  // Left singular vectors
  S: number[];     // Singular values
  Vh: Complex[][]; // Right singular vectors (conjugate transpose)
}

/**
 * Configuration for MPS simulation
 */
export interface MPSConfig {
  maxBondDimension: number;  // Truncation limit for bonds
  truncationThreshold: number; // SVD truncation threshold
  useApproximation: boolean; // Whether to use approximate methods
  enableLargeScale: boolean; // Enable 50+ qubit simulations
  compressionLevel: 'none' | 'light' | 'aggressive'; // Memory/accuracy tradeoff
}

export const DEFAULT_MPS_CONFIG: MPSConfig = {
  maxBondDimension: 64,
  truncationThreshold: 1e-10,
  useApproximation: true,
  enableLargeScale: false,
  compressionLevel: 'light',
};

/**
 * Large-scale configuration for 50-120 qubits
 * Uses more aggressive truncation to maintain tractability
 */
export const LARGE_SCALE_MPS_CONFIG: MPSConfig = {
  maxBondDimension: 128,
  truncationThreshold: 1e-8,
  useApproximation: true,
  enableLargeScale: true,
  compressionLevel: 'aggressive',
};

/**
 * Ultra-scale configuration for 120-210 qubits
 * Very aggressive truncation, lower bond dim for tractability
 */
export const ULTRA_SCALE_MPS_CONFIG: MPSConfig = {
  maxBondDimension: 32,
  truncationThreshold: 1e-6,
  useApproximation: true,
  enableLargeScale: true,
  compressionLevel: 'aggressive',
};

/**
 * Select optimal MPS configuration based on qubit count and gate density
 */
export const getAdaptiveMPSConfig = (qubitCount: number, gateCount: number = 0): MPSConfig => {
  const gateDensity = gateCount / Math.max(qubitCount, 1);

  if (qubitCount <= 20) {
    return {
      ...DEFAULT_MPS_CONFIG,
      maxBondDimension: 128,
      truncationThreshold: 1e-12,
      compressionLevel: 'none',
    };
  }

  if (qubitCount <= 50) {
    return {
      ...DEFAULT_MPS_CONFIG,
      maxBondDimension: gateDensity > 5 ? 64 : 96,
      truncationThreshold: 1e-10,
      compressionLevel: 'light',
    };
  }

  if (qubitCount <= 120) {
    return {
      ...LARGE_SCALE_MPS_CONFIG,
      maxBondDimension: gateDensity > 3 ? 24 : 32,
      truncationThreshold: 1e-8,
    };
  }

  // 120-210 qubits: ultra-scale
  return {
    ...ULTRA_SCALE_MPS_CONFIG,
    maxBondDimension: qubitCount > 170 ? 12 : 16,
    truncationThreshold: qubitCount > 170 ? 1e-5 : 1e-6,
  };
};
